...
try: # 检查是否可用 tqdm，否则定义一个占位版本
    try: # IPython 可能需要不同的 tqdm，使用 try..except 是最简单的兼容方式
        __IPYTHON__
...
from eskf import (
    ...
    ERR_ACC_BIAS_IDX,
    ERR_GYRO_BIAS_IDX,
) # 该类实现了 ESKF 的核心算法，包含预测、更新、NEES/NIS 计算等方法，并定义了状态向量中各个分量的索引

from quaternion import quaternion_to_euler # 该函数可将四元数转换为欧拉角，方便理解和可视化姿态估计结果
from cat_slice import CatSlice # 该工具可用于简化切片操作，如 x[POS_IDX] 等价于 x[POS_IDX[0]:POS_IDX[-1]+1]
...
# 设置绘图样式
try:
    # 通过 "pip install SciencePLots" 安装 (https://github.com/garrettj403/SciencePlots.git)
    # 可获得更美观的图形样式
    import scienceplots
...
# %% 测量噪声
# STIM300 的 IMU 噪声参数，依据数据手册与仿真采样率设置
# 连续时间噪声
# TODO: 可继续调参
...
delta_x_RMSE = np.sqrt(np.mean(delta_x[:N] ** 2, axis=0))  # TODO: 可在图例中使用该 RMSE
axs3[0].plot(t, delta_x[:N, POS_IDX])
axs3[0].set(ylabel="NED position error [m]",xlabel="Time [s]")
axs3[0].legend(
    [
...
axs3[1].set(ylabel="Velocities error [m]",xlabel="Time [s]")
axs3[1].legend(
    [
...
axs3[2].set(ylabel="Euler angles error [deg]",xlabel="Time [s]")
axs3[2].legend(
    [
...
axs3[3].set(ylabel="Accl bias error [m/s^2]",xlabel="Time [s]")
axs3[3].legend(
    [
...
axs3[4].set(ylabel="Gyro bias error [deg/s]",xlabel="Time [s]")
axs3[4].legend(
    [
